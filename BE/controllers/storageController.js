...
export class StorageController {
  ...
  getUploadUrl = asyncHandler(async (req, res) => {
    ...
    const url = await this.gcpService.generateUploadUrl(uniqueFilename, contentType);
  ...
  uploadProxy = asyncHandler(async (req, res) => {
    if (!req.file) {
    ...
    const publicUrl = await this.gcpService.uploadFile(
      uniqueFilename,
      buffer,
      mimetype
    );
